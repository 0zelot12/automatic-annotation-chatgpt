...
for gold_entity in test_document.entities:
    ...
    threshold = math.floor(len(index_range_gold) / 2)
    # print(f"{gold_entity.tokens} -> {threshold}")
    for entity in recognized_entities:
        ...
        if entity.type == gold_entity.type and len(intersection) > threshold:
            print(f"MATCH: {index_range} <-> {index_range_gold}")
            print(f"len(intersection){len(intersection)} > {threshold}")


# for e in test_document.entities:
#     print(f"{e} -> {e.get_index_range()}")


# range_1 = [0, 1, 2, 3, 4]
# range_2 = [2, 3, 4]

# print(np.intersect1d(range_1, range_2))
